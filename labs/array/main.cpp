/*
this program takes an array of numbers and puts it through
various functions
*/
...
void bubbleSort(int nums[], int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		for (j = 0; j < len-i-1; j++) {
			...
			if (nums[j] > nums[j+1]) {
                ...
                swap(nums[j], nums[j+1]); //swaps elements of nums[j] and nums[j+1]
    ...
    for(int z=0; z<len; z++)
        cout<<nums[z]<<" "; //outputs array at end of sort, had trouble doing it in main
    cout<<endl;
}
